...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Generating English lesson plan for:', { teacherName, schoolName, grade, term, strand, subStrand, lessonTitle, lessonNumber });
    ...
    const systemPrompt = `You are an expert Kenyan English Language teacher specializing in the Competency-Based Curriculum (CBC) for Junior Secondary School. Generate lesson plans that are EXACTLY 400 words or less.
...
4. Learning Resources (4-6 items including textbooks, dictionaries, digital devices)
5. Introduction (5 minutes) - Start with a recap of previous lesson, then introduce current lesson objective
6. Lesson Development (25 minutes with 3 numbered steps focusing on language skills)
7. Extended Activities (differentiated for different learner needs)
8. Conclusion (5 minutes with reflection on language skills learned)
...
Use Kenyan context and examples. Focus on practical English language skills including listening, speaking, reading, writing, and grammar.`;

    const userPrompt = `Generate an English lesson plan with these details:
Teacher: ${teacherName}
School: ${schoolName}
Date: ${date}
Grade: ${grade}
...
Create a detailed, engaging lesson plan that develops English language skills effectively. Include practical activities appropriate for Kenyan students.`;
    ...
    console.log('Successfully generated English lesson plan');
